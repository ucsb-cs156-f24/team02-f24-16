...
describe("RecommendationRequestForm tests", () => {
  ...
  test("Correct Error messsages on bad input", async () => {
    ...
    const localDateTimeFieldA = screen.getByTestId("RecommendationRequestForm-dateRequested");
    const localDateTimeFieldB = screen.getByTestId("RecommendationRequestForm-dateNeeded");
    ...
    expect(screen.getByText(/requesterEmail must be in the format <username>@<domain>.<com>/)).toBeInTheDocument();
    expect(screen.getByText(/professorEmail must be in the format <username>@<domain>.<com>/)).toBeInTheDocument();
  ...
  test("Correct Error messsages on missing input", async () => {
    ...
    expect(screen.getByText(/Explanation is required/)).toBeInTheDocument();
    expect(
      screen.queryByText(/requesterEmail must be in the format <username>@<domain>.<com>/),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(/professorEmail must be in the format <username>@<domain>.<com>/),
    ).not.toBeInTheDocument();

    const errorMessages = screen.getAllByText(/Local date time in ISO format required./);
  ...
  test("No Error messsages on good input", async () => {
    ...
    fireEvent.change(localDateTimeFieldA, { target: { value: "2022-01-02T12:00" } });
    fireEvent.change(localDateTimeFieldB, { target: { value: "2023-01-02T12:00" } });
    fireEvent.change(professorEmail,{ target: { value: "aubreydrakegraham@ovo.com" } });
    fireEvent.change(requesterEmail,{ target: { value: "jaydenjardine@cs.ucsb.edu" } });
    fireEvent.change(explanation,{ target: { value: "I need a song with you and Ye ASAP." } });
  ...
  const setup = () => {
    ...
    const emailInput = screen.getByTestId("RecommendationRequestForm-requesterEmail");
    const submitButton = screen.getByTestId("RecommendationRequestForm-submit");
  ...
  validEmails.forEach(email => {
    test(`should accept valid email: ${email}`, async () => {
      ...
      await waitFor(()=>{
        expect(screen.queryByText(/requesterEmail must be in the format <username>@<domain>.<com>/)).not.toBeInTheDocument();
      });
